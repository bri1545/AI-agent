...
export async function generateInterestQuiz(language: string): Promise<{ questions: QuizQuestion[] }> {
  const systemPrompt = `You are an expert in educational psychology and club/activity recommendation for children aged 7-18 at the Palace of Schoolchildren.
...
Respond in JSON format with this structure:
{
  ...
  const userPrompt = `Generate 5 interest quiz questions in ${languageMap[language] || "English"} for students at the Palace of Schoolchildren.`;
...
export async function recommendClubs(interests: string[], availableClubs: any[]): Promise<{ clubId: string; matchPercentage: number }[]> {
  const systemPrompt = `You are an AI assistant that matches student interests to appropriate clubs at the Palace of Schoolchildren.
